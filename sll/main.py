...
if __name__ == '__main__':
  ...
  # resume?
  parser.add_argument('--resume', type=str, help="Checkpoint file")
  ...
  parser.add_argument("--save_checkpoint_epochs", type=int, default=100, help="Save checkpoint every epoch.")
  ...
  parser.add_argument("--mlp", action='store_true', help='Use soft constrained Linear for penultimate layer.')
  parser.add_argument("--liplin", action='store_true', help='Use only soft constrained linear layers. Overrides mlp=True')
  parser.add_argument("--cll_eps", type=float, default=0.15)
  parser.add_argument("--cll_delta", type=float, default=-1, help='If set to negative value, uses cll_eps')
  parser.add_argument("--cll_p", type=float, default=0.1)
  parser.add_argument("--cll_lambda", type=float, default=1.e-5)
  parser.add_argument("--unconstrained_layer_Ks", action='store_true', default=False)
  ...
  if config.cll_delta < 0:
    config.cll_delta = config.cll_eps
  ...
  if config.dataset == 'cifar10':
    os.makedirs('./trained_models_cifar10', exist_ok=True)
    path = realpath('./trained_models_cifar10')
  else:  
